...
class HiddenMarkovModel:
    ...
    def unsupervised_learning(self, X, N_iters, lamda_A, lamda_O):
        '''
        ...
        for iteration in range(1, N_iters + 1):
            ...
            A_num = [[lamda_A / (self.L ** 2) for i in range(self.L)] for j in range(self.L)]
            O_num = [[lamda_O / (self.L * self.D) for i in range(self.D)] for j in range(self.L)]
            A_den = [lamda_A / self.L for i in range(self.L)]
            O_den = [lamda_O / self.L for i in range(self.L)]
            ...
            for x in X:
                ...
                for t in range(1, M):
                    ...
                    # 		O_num[curr][nxt] += lamda_O / (self.L * self.D)
            ...
            # for curr in range(self.L):
            # 	sum_A = sum(A_num[curr])
            # 	for nxt in range(self.L):
            # 		self.A[curr][nxt] = A_num[curr][nxt] / sum_A
            # 	sum_O = sum(O_num[curr])
            # 	for nxt in range(self.D):
            # 		self.O[curr][nxt] = O_num[curr][nxt] / sum_O
...
def unsupervised_HMM(X, n_states, N_iters, n_observations, lamda_A, lamda_O):
    '''
    ...
    HMM = HiddenMarkovModel(A, O)
    HMM.unsupervised_learning(X, N_iters, lamda_A, lamda_O)
